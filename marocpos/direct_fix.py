...
def fix_product_window():
    """Direct fix for product_management_window.py syntax errors"""
    print("Fixing product_management_window.py...")
    
    # Path to the file
    file_path = os.path.join(os.path.dirname(__file__), "ui", "product_management_window.py")
    ...
    # Replace the entire file with a working implementation
    new_content = """
...
"""
    ...
    try:
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        print("Successfully fixed product_management_window.py")
        return True
    except Exception as e:
        print(f"Error writing fixed file: {e}")
        return False

def fix_receipt_generator():
    """Fix QPrinter import in receipt_generator.py"""
    print("\nFixing receipt_generator.py...")
    
    # Path to the file
    file_path = os.path.join(os.path.dirname(__file__), "ui", "receipt_generator.py")
    if not os.path.exists(file_path):
        print(f"ERROR: File not found: {file_path}")
        # Try to locate files with QPrinter import
            ...
            for file in py_files:
                try:
                    ...
                    if "QPrinter" in content:
                        print(f"Found QPrinter in: {os.path.basename(file)}")
                        # Fix this file
                        with open(file, "r", encoding="utf-8") as f:
                            content = f.read()
                        ...
                        print(f"Fixed QPrinter imports in {os.path.basename(file)}")
                except Exception as e:
                    print(f"Error fixing {os.path.basename(file)}: {e}")
        return False
    ...
    try:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(fixed_content)
        print("Successfully fixed receipt_generator.py")
        return True
    except Exception as e:
        print(f"Error fixing receipt_generator.py: {e}")
        return False

if __name__ == "__main__":
    print("=== DIRECT FIX FOR MAROCPOS ===")
    fix_product_window()
    fix_receipt_generator()
    print("\nDone! Please run the application now.")
